          ...
          {mode === null && (
            <div style={{ marginTop: 10 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                ...
                <button onClick={() => setShowMore(!showMore)} style={{ background: "none", border: "none", color: COLORS.sub, fontSize: 13, fontWeight: 600, padding: "9px 4px" }}>
...
const SectionTitle = ({ children }) => <div style={{ fontSize: 12.5, fontWeight: 700, color: COLORS.sub, margin: "20px 0 8px" }}>{children}</div>;
...
function DailyGreeting({ me }) {
  ...
  if (!msg) return null;
  return (
    <div style={{ color: COLORS.sub, padding: "2px 2px 14px", fontSize: 14, lineHeight: 1.45, fontStyle: "italic" }}>
      {msg}
    </div>
  );
}
            ...
            <div style={{ marginTop: 14, borderTop: `1px solid ${COLORS.line}`, paddingTop: 10 }}>
              ...
              {sent.map(n => (
                <div key={n.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8, padding: "6px 0", borderBottom: `1px dashed ${COLORS.line}`, fontSize: 13 }}>
                  ...
                  <button onClick={() => onDelete(n.id)} style={{ border: "none", background: "none", color: COLORS.sub, fontSize: 14, fontWeight: 700, whiteSpace: "nowrap" }}>×</button>
                </div>
              ))}
            </div>
          )}
...
function TodayView({ me, tasks, allTasks, boats, users, isMgr, canAssign, effectiveDeadline, onComplete, onProgress, onExternal, onEdit, onDelete, onChecklistItem, onSetDeadline, onSetDeadlineDuration, onAddBeforePhotos, onLogFinding, onAssign, onAssignWithDeadline, onDowngrade, onTranslate, onHelp, absences, onAddAbsence, onDeleteAbsence, notes, onSendNote, onDeleteNote, onAckExternal, onCloseExternal }) {
  return (
    <div>
      ...
      <div style={{ marginTop: 22, paddingTop: 14, borderTop: `1px solid ${COLORS.line}`, display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 14, marginBottom: 12, flexWrap: "wrap" }}>
          {activeGroup[2].map(([id, label]) => (
            <button key={id} onClick={() => setSection(id)} style={{
              background: "none", border: "none", padding: "2px 0", fontSize: 13,
    ...
    <div style={{ padding: "10px 0", borderTop: `1px dashed ${COLORS.line}` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
        <span style={{ fontSize: 13.5, flex: 1 }}>
          {label}
          {hint && !showHints && (
            <button onClick={() => setOpenHint(!openHint)} style={{ border: "none", background: "none", color: COLORS.sub, fontSize: 12.5, padding: "0 4px" }}>ⓘ</button>
          )}
        </span>
        <span style={{ flexShrink: 0 }}>{children}</span>
      </div>
      {hint && visible && <div style={{ fontSize: 11.5, color: COLORS.sub, marginTop: 3 }}>{hint}</div>}
    </div>
  );
}
