...
// ή παραλείπεται· μεμονωμένο ⚠ γίνεται αυτόματα εργασία, όπως ακριβώς στο checklist κλεισίματος.
// Είναι η ΒΑΣΙΚΗ λίστα της βάσης — κάθε σκάφος μπορεί να έχει δικές του προσθαφαιρέσεις (boat.inventoryExtra /
// boat.inventoryHidden), γιατί δεν έχουν όλα τα σκάφη bow thruster, γεννήτρια κ.λπ.
const INVENTORY_CATS = [
  ["deck", "Κατάστρωμα & Cockpit"],
  ["locker1", "Locker 1"],
  ["locker2", "Locker 2"],
  ["linen", "Λευκά είδη"],
  ["sails", "Πανιά"],
...
// Μόνο για αγγλικές εξαγωγές PDF — οι κατηγορίες μέσα στην εφαρμογή μένουν πάντα ελληνικά.
const INVENTORY_CAT_LABEL_EN = {
  deck: "Deck & Cockpit", locker1: "Locker 1", locker2: "Locker 2",
  linen: "Linens", sails: "Sails", safety: "Safety", nav: "Navigation",
};
const SEED_INVENTORY = {
  deck: ["Bimini", "Spray Hood", "Βιτζιρέλα (Winches)", "Μανέλες", "Πασαρέλα", "Πλατφόρμα", "Ντους καταστρώματος", "Τραπέζι cockpit", "Μαξιλάρια cockpit", "Χερούλι αντλίας", "Χειροκίνητη αντλία", "1 Σωσίβια λέμβος", "Μπαλόνια", "2 Πυξίδες", "Φως cockpit", "Άγκυρα", "Αλυσίδα άγκυρας", "Εργάτης άγκυρας", "Μανέλα εργάτη", "1 Βάρκα", "Κολονάκια", "1 Βυθόμετρο", "1 Ταχύμετρο", "1 Ανεμόμετρο", "Αυτόματος πιλότος", "1 GPS", "Προωστήρας πλώρης"],
  locker1: ["1 Τρόμπα ποδιού", "1 Τρόμπα χεριού", "1 Εφεδρική άγκυρα", "1 Εφεδρικό πηδάλιο", "1 Βούρτσα καταστρώματος", "1 Χοάνη καυσίμων", "1 Σχοινί 50 μ.", "2 Σχοινιά 20 μ.", "2 Σχοινιά 10 μ.", "1 Κόφτης", "Βατραχοπέδιλα", "Μάσκα", "2 Σωσίβια πέταλα", "Ανατρεπόμενη λυχνία"],
  locker2: ["1 Λάστιχο νερού", "1 Αντάπτορας λάστιχου", "1 Πλαστικός κουβάς", "1 Λάδι μηχανής", "1 Παραφλού / αντιψυκτικό", "2 Φιάλες αερίου", "2 Πυροσβεστήρες", "Δοχείο βενζίνης (άδειο)", "Καλώδιο εξωτ. παροχής ρεύματος", "1 Αντάπτορας 220V", "2 Κουπιά για τη βάρκα", "1 Γάντζος", "1 Σκάλα κολύμβησης"],
  linen: ["Κουβέρτες", "Μαξιλάρια", "Σεντόνια", "Πετσέτες", "Μαξιλαροθήκες"],
  sails: ["Μαΐστρα", "Σάκος μαΐστρας (Lazy Jack)", "Φλόκος (Genoa)"],
  safety: ["Σωσίβια (ενηλίκων)", "Σωσίβια (παιδικά)", "Ζώνες ασφαλείας", "1 Φωτοβολίδα (σαλόνι)", "1 Καρεκλάκι ανύψωσης (σαλόνι)", "1 Μαύρη μπάλα αγκυροβολίας (σαλόνι)", "Κουτί πρώτων βοηθειών (σαλόνι)", "EPIRB", "Πυροσβεστήρας", "Κόρνα ομίχλης", "Πλωτή άγκυρα", "Ξύλινες σφήνες", "1 Εργαλειοθήκη", "Σετ επισκευής λέμβου", "Σετ επισκευής πανιών", "Ανταλλακτικά μηχανής"],
  nav: ["1 Πυξίδα χειρός", "1 Διαβαθμισμένος χάρακας", "1 Κουμπάσο", "Χάρτες", "1 Greek Water Pilot", "1 Ζευγάρι κιάλια", "1 Φακός", "1 VHF", "1 CD Player"],
};
// Η λίστα του συγκεκριμένου σκάφους = βασική λίστα βάσης − όσα δεν έχει αυτό το σκάφος + όσα έχει επιπλέον.
const buildInventoryItems = (boat, base) => {
  const hidden = new Set(boat?.inventoryHidden || []);
  const extra = boat?.inventoryExtra || {};
  const out = [];
  INVENTORY_CATS.forEach(([cat]) => {
    const items = [...((base && base[cat]) || []), ...((extra && extra[cat]) || [])];
    items.forEach((text, i) => {
      if (hidden.has(`${cat}::${text}`)) return;
      out.push({ id: `inv-${cat}-${i}-${Math.random().toString(36).slice(2, 5)}`, cat, text, status: "pending", problemTaskId: null, note: "" });
    });
  });
  return out;
};
...
const DEFAULT_SETTINGS = {
  ...
  // Ε. Φωτογραφίες
  // Inventory List — έλεγχος εξοπλισμού πριν την παράδοση στον πελάτη
  autoInventory: true,            // δημιουργείται μόνο του πριν από κάθε αναχώρηση
  inventoryDaysBefore: 2,         // πόσες μέρες πριν την αναχώρηση εμφανίζεται
...
// Τα αυτόματα μηνύματα «κλείσιμο σκάφους» (kind="closing-alert") και «απόρριψη εργασίας» (kind="decline")
// έχουν δικό τους κανόνα: μένουν ορατά μέχρι τις 11:00 το επόμενο πρωί από την αποστολή τους — αρκετός χρόνος
// να τα δει κανείς το βράδυ, αλλά χωρίς να γίνονται «θόρυβος» στην πρώτη εικόνα των εργασιών της επόμενης μέρας.
const isNoteVisible = (n) => {
  if (n.kind === "closing-alert" || n.kind === "decline") {
...
// Μία κλήση AI για ολόκληρη τη λίστα αντί για μία ανά γραμμή — ένα export μπορεί να έχει δεκάδες γραμμές
// (εργασίες + παρατηρήσεις + inventory/check-in μαζί) και δεν έχει νόημα ξεχωριστό round-trip για καθεμία.
// Αν αποτύχει η μετάφραση (δίκτυο/parsing), επιστρέφει τα πρωτότυπα ελληνικά αντί να μπλοκάρει την εκτύπωση.
async function translateBatchToEnglish(texts) {
  const clean = texts.map(t => (t || "").toString());
  if (!clean.some(t => t.trim())) return clean;
  try {
    const prompt = `Translate each of the following boat-maintenance/charter-related lines from Greek to English, in natural nautical/technical style. Reply with ONLY a JSON array of strings, same length and same order as the input, no explanation:\n\n${JSON.stringify(clean)}`;
    const out = await askClaude(prompt, Math.min(4000, 200 + clean.join("").length * 2));
    const match = out.match(/\[[\s\S]*\]/);
    const arr = JSON.parse(match ? match[0] : out);
    if (Array.isArray(arr) && arr.length === clean.length) return arr.map(String);
  } catch {}
  return clean;
}
...
function AppInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (settings.autoInventory) freshTasks = await generateInventoryChecks(freshTasks);
...
Βάλε intensive:true για τύπους «δείξε τη δουλειά σου» (καθάρισμα/γυάλισμα/λεύκανση). Βάλε findMode:true ΜΟΝΟ για τον τύπο «ΕΝΤΟΠΙΣΕ ΤΟΥΛΑΧΙΣΤΟΝ 3».`;
  ...
  // συνδεδεμένο manager — έτσι το «Προβολή ως» είναι ΠΡΑΓΜΑΤΙΚΗ προσομοίωση: βλέπεις ακριβώς ό,τι θα έβλεπε ο
  // υπάλληλος, χωρίς κρυφά προνόμια διαχειριστή να μπερδεύουν την κρίση για το τι χρειάζεται διόρθωση στην
  // εφαρμογή. Η έξοδος από το «Προβολή ως» (μπάρα «Επιστροφή σε {me.name}») δεν εξαρτάται από αυτό — δουλεύει
  // πάντα, ό,τι κι αν δείχνει η acting.role στιγμιαία.
...
Δώσε ΣΥΝΤΟΜΗ πρακτική πρόταση λύσης (2-4 προτάσεις)${extraText ? ", προσαρμοσμένη ειδικά στα συμπτώματα που περιγράφηκαν" : ""}. Αν το ιστορικό δείχνει τι δούλεψε ξανά σε παρόμοιο πρόβλημα, ανάφερέ το ΠΡΩΤΟ και ρητά. Αν δεν υπάρχει σχετικό ιστορικό, χρησιμοποίησε τη γενική τεχνική σου γνώση για σκάφη/θαλάσσιο εξοπλισμό. Μίλα απευθείας, χωρίς εισαγωγικές φράσεις τύπου "Ορίστε η πρόταση".`;
  ...
  // στο «Δεν μπορώ» για να αποφασιστεί αν ειδοποιείται κάποιος manager ή αν το AI ψάχνει αναπλήρωση.
  const SYSTEM_ASSIGNERS = ["AI", "AI-backlog", "auto-closing", "auto-purchase"];
  // «Δεν μπορώ»: ο ίδιος ο υπεύθυνος απελευθερώνει μια εργασία που του έχει ανατεθεί, με υποχρεωτική αιτιολογία.
  // Η εργασία γίνεται ΠΑΝΤΑ πλήρως ελεύθερη (χωρίς υπεύθυνο ΟΥΤΕ βοηθούς) — σκόπιμα διαφορετικό από το reassign()
  // που προάγει βοηθό σε υπεύθυνο, γιατί εδώ πρόκειται για ρητή δήλωση αδυναμίας, όχι απλή αλλαγή υπευθύνου.
  // Η αιτιολογία μένει καταγεγραμμένη πάνω στην εργασία (declines[]) — ορατή σε όποιον την αναλάβει μετά.
  const declineTask = async (t, reason) => {
    ...
    if (SYSTEM_ASSIGNERS.includes(prevAssignedBy)) {
      ...
      // για το ίδιο άτομο, ώστε να μη μείνει άπρακτο επειδή απλώς δεν ταίριαζε η συγκεκριμένη εργασία.
      assignReplacementTask(nextTasks, decliner, t.id);
    } else {
      // Ανατέθηκε από συγκεκριμένο manager/στέλεχος: ειδοποιείται εκείνος με το ποιος αρνήθηκε και γιατί.
      // Αν κάποιος είχε αναθέσει την εργασία στον εαυτό του, δεν στέλνεται μήνυμα (δεν έχει νόημα).
      const assigner = users.find(u => u.id === prevAssignedBy);
      if (assigner && assigner.id !== decliner) {
        await sendNote([assigner.id], `❌ Ο/Η ${acting.name} δεν μπορεί να αναλάβει «${t.desc}» — αιτία: ${reason}`, "system", "decline");
      }
    }
  };
  // Στοχευμένη αναπλήρωση μετά από «Δεν μπορώ»: ΜΙΑ μικρή κλήση AI για ΕΝΑ συγκεκριμένο άτομο (όχι πλήρης
  // ανακατανομή όλης της ημέρας) — ψάχνει την πιο κατάλληλη ελεύθερη εργασία γι' αυτόν, αλλιώς δεν κάνει τίποτα.
  const assignReplacementTask = async (tasksSrc, userId, excludeTaskId) => {
    ...
    try {
      const boatName = (id) => boats.find(b => b.id === id)?.name || "Βάση/Άλλο";
      const prompt = `Ένας υπάλληλος μόλις δήλωσε αδυναμία για μια εργασία που του είχε ανατεθεί αυτόματα και έμεινε χωρίς. Διάλεξε ΤΗΝ ΠΙΟ ΚΑΤΑΛΛΗΛΗ ελεύθερη εργασία γι' αυτόν από τη λίστα — ή καμία, αν καμία δεν ταιριάζει πραγματικά.
Υπάλληλος: ${emp.name} (προφίλ: ${emp.profile || "χωρίς προφίλ"})
Ελεύθερες εργασίες: ${free.map(x => `${x.id}: "${x.desc}" [σκάφος: ${boatName(x.boatId)}${x.urgent ? ", ΕΠΕΙΓΟΝ" : ""}]`).join("; ")}
Απάντησε ΜΟΝΟ με JSON, χωρίς markdown: {"taskId": "..." ή null}`;
  ...
  const resolveInventoryItem = async (task, itemId, outcome, note) => {
    ...
    if (outcome === "problem") {
      newTaskId = "t" + Date.now() + "-ip";
      const item = (task.inventoryItems || []).find(it => it.id === itemId);
      extraTask = {
...
function InventoryItems({ t, boat, onInventoryItem, onBulkCategory, onFinish, onConfirm, isMgr, users }) {
  const [exportEnglish, setExportEnglish] = useState(false);
  const [translating, setTranslating] = useState(false);
  const [printPayload, setPrintPayload] = useState(null);
  const startExport = async (pendingItems) => {
    const lang = exportEnglish ? "en" : "el";
    let texts = pendingItems.map(it => it.text);
    if (exportEnglish) { setTranslating(true); texts = await translateBatchToEnglish(texts); setTranslating(false); }
    const catLabel = lang === "en" ? INVENTORY_CAT_LABEL_EN : INVENTORY_CAT_LABEL;
    setPrintPayload({ lang, items: pendingItems.map((it, i) => ({ id: it.id, text: `${catLabel[it.cat]}: ${texts[i]}` })) });
  };
  useEffect(() => {
    ...
    const reset = () => { setPrintPayload(null); document.title = prevTitle; };
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
        ...
        {!done && pending > 0 && (
          <div style={{ display: "flex", gap: 4 }}>
            ...
            {boat && (
              <>
                <Btn small color={exportEnglish ? COLORS.navy : COLORS.sub} outline={!exportEnglish} onClick={() => setExportEnglish(v => !v)}>🇬🇧</Btn>
                <Btn small color={COLORS.sub} outline onClick={() => startExport(items.filter(it => it.status === "pending"))}>{translating ? "Μετάφραση…" : "🖨 Εξαγωγή εκκρεμών"}</Btn>
              </>
            )}
          </div>
        )}
        ...
        return (
          <div key={cat} style={{ border: `1px solid ${COLORS.line}`, borderRadius: R.sm, marginBottom: 8 }}>
            ...
            <div style={{ display: "flex", gap: 8, padding: "0 12px 12px" }}>
              <Btn small color={COLORS.green} outline onClick={() => onBulkCategory(t, cat, "ok")}>Όλα OK</Btn>
              <Btn small color={NEUTRAL} outline onClick={() => onBulkCategory(t, cat, "skipped")}>Παράλειψη</Btn>
            </div>
            {isOpen && (
      ...
      {!done && (
        <div style={{ marginTop: 12 }}>
          <Btn color={COLORS.green} onClick={() => onFinish(t)}>Ολοκλήρωση inventory</Btn>
      ...
      {done && (
        <div style={{ marginTop: 8, paddingTop: 8, borderTop: `1px solid ${COLORS.line}` }}>
          ...
          {t.inventoryConfirmedBy
            ? <div style={{ fontSize: T.caption, color: COLORS.green, fontWeight: 600, marginTop: 4 }}>✔ Επιβεβαιώθηκε από {un(t.inventoryConfirmedBy)}</div>
            : isMgr && <div style={{ marginTop: 8 }}><Btn small color={COLORS.navy} onClick={() => onConfirm(t)}>Επιβεβαίωση</Btn></div>}
        </div>
      )}
      {printPayload && (
        <PendingItemsPrintSheet boat={boat} title={printPayload.lang === "en" ? "Inventory — pending" : "Inventory — εκκρεμή"}
          items={printPayload.items} lang={printPayload.lang} />
      )}
    </div>
  );
}
...
function CheckinItems({ t, boat, onCheckinItem, onFinish, isMgr, users }) {
  ...
  const startExport = async (pendingItems) => {
    ...
    let texts = pendingItems.map(it => it.text);
    if (exportEnglish) { setTranslating(true); texts = await translateBatchToEnglish(texts); setTranslating(false); }
    setPrintPayload({ lang, items: pendingItems.map((it, i) => ({ id: it.id, text: texts[i] })) });
  };
  useEffect(() => {
    if (!printPayload) return;
    let cancelled = false;
  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      {printPayload && (
        <PendingItemsPrintSheet boat={boat} title={printPayload.lang === "en" ? "Check-in — pending" : "Check-in — εκκρεμή"}
          items={printPayload.items} lang={printPayload.lang} />
      )}
    </div>
  );
    ...
    <div style={{
      ...
      {open && (
        <div style={{ padding: "0 12px 12px", borderTop: `1px solid ${COLORS.line}` }}>
          ...
          {t.declines?.length > 0 && (
            <div style={{ background: "#FBEBE6", color: "#8A2A1C", padding: "8px 8px", borderRadius: 8, fontSize: 13, margin: "8px 0" }}>
              <b>⚠ {tr("Απορρίφθηκε πριν:")}</b>
              {t.declines.map((d, i) => (
                <div key={i} style={{ marginTop: 3 }}>{users.find(u => u.id === d.by)?.name || "—"}: «{d.reason}» — {fmtDate(d.at)}</div>
              ))}
            </div>
          )}
          {t.photos?.length > 0 && (
            ...
            <div style={{ marginTop: 8 }}>
              ...
              {showMore && (
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8, paddingTop: 8, borderTop: `1px dashed ${COLORS.line}` }}>
                  ...
                  {onDecline && t.assignedTo === me?.id && t.status === "open" && <Btn small color={COLORS.red} outline onClick={() => { setMode("decline"); setNote(""); }}>{tr("Δεν μπορώ")}</Btn>}
          ...
          {mode === "decline" && (
            <div style={{ marginTop: 8, background: "#FBEBE6", borderRadius: 12, padding: 12 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#8A2A1C", marginBottom: 8 }}>{tr("Γιατί δεν μπορείς να αναλάβεις αυτή την εργασία;")}</div>
              <textarea value={note} onChange={e => setNote(e.target.value)} rows={2} placeholder={tr("π.χ. δεν έχω τα εργαλεία / έχω άλλη προτεραιότητα σήμερα")} style={inputStyle} />
              <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                <Btn small color={COLORS.red} onClick={() => { if (!note.trim()) return; onDecline(t, note.trim()); setMode(null); setNote(""); }}>{tr("Επιβεβαίωση")}</Btn>
                <Btn small color={COLORS.sub} outline onClick={() => setMode(null)}>{tr("Άκυρο")}</Btn>
              </div>
            </div>
          )}
          {mode === "deadline" && (
            <div style={{ marginTop: 8 }}>
              ...
              {onToggleExcludeDeadline && (
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, marginTop: 12, paddingTop: 10, borderTop: `1px dashed ${COLORS.line}` }}>
                  <div>
                    <div style={{ fontSize: 13 }}>{tr("Χωρίς πίεση χρόνου")}</div>
                    <div style={{ fontSize: 12, color: COLORS.sub, marginTop: 2 }}>{tr("Δεν θα γίνεται ποτέ επείγουσα ούτε θα ανεβαίνει ψηλά όταν πλησιάζει αναχώρηση — π.χ. αισθητική δουλειά χαμηλής σοβαρότητας.")}</div>
                  </div>
                  <Toggle on={!t.excludedFromDeadline} onChange={() => onToggleExcludeDeadline(t)} />
                </div>
              )}
              <div style={{ marginTop: 8 }}>
                <Btn small color={COLORS.sub} outline onClick={() => setMode(null)}>{tr("Άκυρο")}</Btn>
...
function FleetScheduleWidget({ boats, allTasks, onBoatClick, onQuickInventory, isMgr, onResetInventory }) {
  const [open, setOpen] = useState(false);
  ...
  return (
    <div style={{ background: COLORS.card, borderRadius: R.lg, padding: 12, marginBottom: 12, border: `1px solid ${COLORS.line}` }}>
      ...
      {open && (
        <div style={{ marginTop: 8 }}>
          {ordered.map(({ b, returnDate, returnDays, departDate, departDays, openCount, invOpen, invDone }, i) => {
            ...
            return (
              <div key={b.id} role={onBoatClick ? "button" : undefined} tabIndex={onBoatClick ? 0 : undefined} onClick={() => onBoatClick && onBoatClick(b.id)} style={{
                ...
                {/* Ετοιμότητα παράδοσης με μια ματιά — αυτό που δεν έλυνε ποτέ το χαρτί. */}
                {(invOpen || invDone || departLabel) && (
                  <div style={{ marginTop: 2, fontSize: T.caption }}>
                    {invOpen
                      ...
                      : invDone
                        ? <span>
                            ...
                            {isMgr && onResetInventory && (
                              <button onClick={(e) => { e.stopPropagation(); onResetInventory(invDone); }} style={{
                                marginLeft: 8, color: COLORS.red, fontSize: T.caption, fontWeight: 600, background: "none", border: "none", padding: 0,
    ...
    <div>
      ...
      {invPending.length > 0 && (
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.line}`, borderRadius: 12, padding: 12, marginBottom: 12 }}>
          <div style={{ fontWeight: 700, fontSize: T.small, marginBottom: 4 }}>Inventory προς επιβεβαίωση ({invPending.length})</div>
          {invPending.map(t => {
            const probs = (t.inventoryItems || []).filter(it => it.status === "problem").length;
            return (
              <div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, padding: "8px 0", borderTop: `1px dashed ${COLORS.line}` }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: T.small, fontWeight: 600 }}>{bn(t.boatId)}</div>
                  <div style={{ fontSize: T.caption, color: probs > 0 ? COLORS.red : COLORS.sub, marginTop: 2 }}>
                    {un(t.completedBy) || "—"} · {fmtDate(t.completedAt)}{probs > 0 ? ` · ⚠ ${probs} πρόβλημα(τα)` : " · όλα εντάξει"}
                  </div>
                </div>
                <Btn small color={COLORS.navy} onClick={() => onConfirmInventory(t)}>Επιβεβαίωση</Btn>
              </div>
            );
          })}
        </div>
...
function BoatDetail({ boat, tasks, boatNotes, onAddNote, onDeleteNote, onClearNotes, partners, isMgr, isOwner, persistBoats, showToast, onExportBoat, onDeleteBoat }) {
  ...
  const [expObservations, setExpObservations] = useState(false);
  const [expFrom, setExpFrom] = useState("");
  const [expTo, setExpTo] = useState("");
  const [expCompany, setExpCompany] = useState("");
  const [expNote, setExpNote] = useState("");
  const [expEnglish, setExpEnglish] = useState(false);
...
Απάντησε σύντομα και συγκεκριμένα στα ελληνικά.`;
  ...
  return (
    <div style={{ marginTop: 8, background: COLORS.bg, borderRadius: 12, padding: 12 }}>
      ...
      {onExportBoat && (
        <InfoSection title="Εξαγωγή PDF" hint="Φτιάξε ένα PDF με ό,τι διαλέξεις — εργασίες, ό,τι δεν έχει γίνει τσεκ στο Check-in/Inventory, ή τις παρατηρήσεις σου.">
          ...
          {exportOpen && (
            <div style={{ marginTop: 8, background: COLORS.card, borderRadius: 8, padding: 8 }}>
              ...
              {isOwner && (
                <label style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0", fontSize: 14 }}>
                  <input type="checkbox" checked={expObservations} onChange={e => setExpObservations(e.target.checked)} /> Παρατηρήσεις σου
                </label>
              )}

              {isOwner && (
                <div style={{ marginTop: 8, borderTop: `1px dashed ${COLORS.line}`, paddingTop: 8 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: COLORS.sub, marginBottom: 6 }}>Στοιχεία συνεργασίας (προαιρετικό — μόνο για σένα, όχι για base manager)</div>
                  <div style={{ display: "flex", gap: 4, alignItems: "center", flexWrap: "wrap", marginBottom: 8 }}>
                    <input type="date" value={expFrom} onChange={e => setExpFrom(e.target.value)} style={{ ...inputStyle, width: "auto" }} placeholder="Ημερομηνία ναύλου" />
                    <span style={{ color: COLORS.sub }}>→</span>
                    <input type="date" min={expFrom} value={expTo} onChange={e => setExpTo(e.target.value)} style={{ ...inputStyle, width: "auto" }} />
                  </div>
                  <input value={expCompany} onChange={e => setExpCompany(e.target.value)} placeholder="Εταιρεία συνεργασίας" list="obs-company-suggestions" style={{ ...inputStyle, marginBottom: 8 }} />
                  <datalist id="obs-company-suggestions">{partnerCompanies.map(c => <option key={c} value={c} />)}</datalist>
                  <textarea value={expNote} onChange={e => setExpNote(e.target.value)} rows={2} placeholder="Γραπτή παρατήρηση για αυτό το PDF (προαιρετικό)" style={inputStyle} />
                </div>
              )}

              <label style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0", fontSize: 14, marginTop: 8, borderTop: `1px dashed ${COLORS.line}`, paddingTop: 8 }}>
                <input type="checkbox" checked={expEnglish} onChange={e => setExpEnglish(e.target.checked)} /> 🇬🇧 Στα αγγλικά (η εφαρμογή μένει στα ελληνικά)
              </label>
              <div style={{ marginTop: 8 }}>
                <Btn small color={COLORS.navy} onClick={async () => {
                  if (!expTasks && !expCheckin && !expInventory && !expObservations) { showToast?.("Επίλεξε τουλάχιστον ένα"); return; }
                  setExportBusy(true);
                  await onExportBoat({
                    tasks: expTasks, checkin: expCheckin, inventory: expInventory, observations: isOwner && expObservations,
                    charterFrom: isOwner ? expFrom : "", charterTo: isOwner ? expTo : "", company: isOwner ? expCompany : "", note: isOwner ? expNote : "",
                    english: expEnglish,
                  });
                  setExportBusy(false); setExportOpen(false);
                }}>{exportBusy ? "Μετάφραση…" : "🖨 Εκτύπωση"}</Btn>
              </div>
            </div>
          )}
        </InfoSection>
      ...
      {isOwner && (
        <InfoSection title="Παρατηρήσεις καπετάνιου" hint="Θετικές ή αρνητικές, ό,τι αξίζει να θυμάσαι για αυτό το σκάφος — ορατές μόνο σε σένα, σε κανέναν άλλον. Η εκτύπωσή τους γίνεται πλέον από την «Εξαγωγή PDF» παραπάνω.">
          {onClearNotes && myNotes.length > 0 && (
            <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 8 }}>
              <Btn small color={COLORS.red} outline onClick={() => setConfirmClearNotes(true)}>🗑 Διαγραφή όλων</Btn>
            </div>
          )}
          {confirmClearNotes && (
...
// check-in εκκρεμή / inventory εκκρεμή / παρατηρήσεις), σε οποιονδήποτε συνδυασμό, και όλα μπαίνουν στο ίδιο
// PDF κάτω από ένα κοινό header σκάφους. Το `meta` (καπετάνιος/ημερομηνία ναύλου/εταιρεία/γραπτή σημείωση)
// είναι προαιρετικό και εμφανίζεται μόνο όταν υπάρχει πραγματικά κάτι μέσα — δηλαδή μόνο όταν ο ιδιοκτήτης το
...
function BoatExportPrintSheet({ boat, sections, lang, meta }) {
  ...
  return (
    <div className="print-area" style={{ fontFamily: FONT_STACK, color: "#111", background: "#fff", padding: "28px 34px", boxSizing: "border-box" }}>
      ...
      {meta && (
        <div style={{ display: "flex", gap: 24, flexWrap: "wrap", fontSize: 13, color: "#333", marginBottom: meta.note ? 14 : 22, paddingBottom: 14, borderBottom: meta.note ? "none" : "1px solid #ddd" }}>
          <div><span style={{ color: "#888" }}>{en ? "Captain: " : "Καπετάνιος: "}</span><b>{meta.captainName || "—"}</b></div>
          <div><span style={{ color: "#888" }}>{en ? "Charter date: " : "Ημερομηνία ναύλου: "}</span><b>{charterRange}</b></div>
          <div><span style={{ color: "#888" }}>{en ? "Company: " : "Εταιρεία: "}</span><b>{meta.company || "—"}</b></div>
        </div>
      )}
      {meta?.note && (
        <div style={{ fontSize: 14, lineHeight: 1.5, whiteSpace: "pre-wrap", background: "#F7F6F3", borderRadius: 8, padding: "10px 12px", marginBottom: 22 }}>{meta.note}</div>
      )}
      {sections.map((s, si) => (
          ...
          <PrintNumberedList items={s.items} emptyText={s.emptyText} renderItem={it => (
            <>
              <div style={{ fontSize: 15, lineHeight: 1.5 }}>
                {it.text}
                {it.urgent && <span style={{ marginLeft: 8, fontSize: 11, fontWeight: 800, color: "#8A1F1F", border: "1px solid #8A1F1F", borderRadius: 4, padding: "1px 6px" }}>{lang === "en" ? "URGENT" : "ΕΠΕΙΓΟΝ"}</span>}
              </div>
              {it.date && <div style={{ fontSize: 11, color: "#888", marginTop: 2 }}>{fmtDate(it.date)}</div>}
...
// ξεχωρίζει ή να ονομάζεται τίποτα «εργασία»: όταν αυτό φεύγει σε άλλη εταιρεία δεν δίνει εντολές στο
// προσωπικό της, απλά την ενημερώνει τι έχει βρει. Η ίδια πληροφορία λέγεται «εργασίες» μόνο στο δικό του
// ξεχωριστό section (BoatExportPrintSheet, key "tasks").
...
function BoatsAdmin({ boats, isOwner, me, tasks, boatNotes, onAddBoatNote, onDeleteBoatNote, onClearBoatNotes, partners, isMgr, persistBoats, onStartInventory, onStartCheckin, showToast }) {
  ...
  // ο ίδιος ποια sections (εργασίες / check-in εκκρεμή / inventory εκκρεμή / παρατηρήσεις) θέλει στο ίδιο PDF,
  // οποιονδήποτε συνδυασμό. `null` σε ένα section σημαίνει «δεν το ζήτησε» (δεν εμφανίζεται καθόλου) — []
  // σημαίνει «το ζήτησε αλλά δεν έχει τίποτα» (εμφανίζεται με «καμία εκκρεμότητα»).
  // Τα στοιχεία συνεργασίας (ημερομηνία ναύλου/εταιρεία/γραπτή σημείωση) είναι δυνατότητα ΜΟΝΟ του ιδιοκτήτη —
  // ένας base manager χρησιμοποιεί το ίδιο κουμπί αλλά ούτε καν βλέπει αυτά τα πεδία στη φόρμα (BoatDetail),
  // οπότε εδώ φτάνουν πάντα κενά για εκείνον και το `meta` παρακάτω βγαίνει null — καμία ανάμειξη ρόλων.
  const [exportBoat, setExportBoat] = useState(null);
  // Μαζεύει τα δεδομένα ΠΡΙΝ μπει το print-area στο DOM (όχι reactively μέσα στο render) ώστε να προλάβει να
  // περιμένει τη μετάφραση όταν ζητηθεί αγγλικά — μία κλήση AI για όλο το κείμενο του export μαζί, όχι ανά section.
  const exportBoatForPrint = async (boat, opts) => {
    const taskItems = !opts.tasks ? [] : tasks.filter(t => t.boatId === boat.id && t.status === "open" && !t.inventoryItems && !t.checkinItems);
    ...
    const lang = opts.english ? "en" : "el";
    if (opts.english) {
      const all = [...taskTexts, ...checkinTexts, ...invTexts, ...noteTexts, ...obsTaskTexts, ...(freeNote ? [freeNote] : [])];
      const translated = await translateBatchToEnglish(all);
      let i = 0;
      taskTexts = taskTexts.map(() => translated[i++]);
      checkinTexts = checkinTexts.map(() => translated[i++]);
      invTexts = invTexts.map(() => translated[i++]);
      noteTexts = noteTexts.map(() => translated[i++]);
      obsTaskTexts = obsTaskTexts.map(() => translated[i++]);
      if (freeNote) freeNote = translated[i++];
    }
    const catLabel = lang === "en" ? INVENTORY_CAT_LABEL_EN : INVENTORY_CAT_LABEL;
    const sections = [
      !opts.tasks ? null : { key: "tasks", title: lang === "en" ? "Tasks" : "Εργασίες", emptyText: lang === "en" ? "No open tasks." : "Καμία ανοιχτή εργασία.",
        items: taskItems.map((t, i) => ({ id: t.id, text: taskTexts[i], urgent: t.urgent, photos: t.photos })) },
      !opts.checkin ? null : { key: "checkin", title: lang === "en" ? "Check-in — pending" : "Check-in — εκκρεμή", emptyText: lang === "en" ? "Nothing pending." : "Τίποτα εκκρεμές.",
        items: checkinItems.map((it, i) => ({ id: it.id, text: checkinTexts[i] })) },
      !opts.inventory ? null : { key: "inventory", title: lang === "en" ? "Inventory — pending" : "Inventory — εκκρεμή", emptyText: lang === "en" ? "Nothing pending." : "Τίποτα εκκρεμές.",
        items: invItems.map((it, i) => ({ id: it.id, text: `${catLabel[it.cat]}: ${invTexts[i]}` })) },
      !opts.observations ? null : { key: "observations", title: lang === "en" ? "Observations" : "Παρατηρήσεις", emptyText: lang === "en" ? "No observations logged." : "Καμία παρατήρηση καταχωρημένη.",
        items: combinedObservations(rawNotes.map((n, i) => ({ ...n, text: noteTexts[i] })), rawObsTasks.map((t, i) => ({ ...t, desc: obsTaskTexts[i] }))) },
    ].filter(Boolean);
    const hasCompanyInfo = opts.charterFrom || opts.charterTo || opts.company || opts.note;
    const meta = !hasCompanyInfo ? null : { captainName: me?.name, charterFrom: opts.charterFrom, charterTo: opts.charterTo, company: opts.company, note: freeNote };
    setExportBoat({ boat, lang, sections, meta });
  };
  useEffect(() => {
  ...
  return (
    <div>
      ...
      <BoatExportPrintSheet boat={exportBoat?.boat || null} sections={exportBoat?.sections || []} lang={exportBoat?.lang} meta={exportBoat?.meta} />
    </div>
  );
}
