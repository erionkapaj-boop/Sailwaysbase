  ...
  return (
    <div style={{ background: COLORS.card, borderRadius: 12, padding: 14, marginBottom: 14, border: `1px solid ${COLORS.line}` }}>
      <button onClick={() => setOpen(!open)} style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", background: "none", border: "none", padding: 0, textAlign: "left" }}>
        <span style={{ fontWeight: 800, fontSize: 14 }}>⚓ Αναχωρήσεις & Επιστροφές ({winDays} μέρες)</span>
...
function AdminView(props) {
  ...
  const sections = [
    ...
    ["profiles", "Ομάδα"], ["settings", "⚙️ Ρυθμίσεις"], ["trash", `Κάδος${tasksRaw?.length ? ` (${tasksRaw.length})` : ""}`],
...
function SettingsGroup({ title, subtitle, children, openKey, cur, setCur }) {
  ...
  return (
    <div style={{ background: COLORS.card, borderRadius: 12, marginBottom: 8, border: `1px solid ${isOpen ? COLORS.navy : COLORS.line}` }}>
      <button onClick={() => setCur(isOpen ? null : openKey)} style={{ width: "100%", background: "none", border: "none", padding: "13px 14px", textAlign: "left", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
        <span>
          <span style={{ fontWeight: 700, fontSize: 14.5, color: COLORS.text }}>{title}</span>
          {subtitle && <span style={{ display: "block", fontSize: 11.5, color: COLORS.sub, marginTop: 2 }}>{subtitle}</span>}
        </span>
...
function SettingRow({ label, hint, children }) {
  return (
    <div style={{ padding: "10px 0", borderTop: `1px dashed ${COLORS.line}` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
        <span style={{ fontSize: 13.5, flex: 1 }}>{label}</span>
        <span style={{ flexShrink: 0 }}>{children}</span>
      </div>
      {hint && <div style={{ fontSize: 11.5, color: COLORS.sub, marginTop: 3 }}>{hint}</div>}
    </div>
  );
}
function Toggle({ on, onChange }) {
  return (
    <button onClick={() => onChange(!on)} style={{
      padding: "5px 12px", borderRadius: 999, fontSize: 12, fontWeight: 700, minWidth: 68,
...
function SettingsAdmin({ settings, updateSettings, resetSettings }) {
  ...
  const hourIn = (key) => (
    <select value={s[key]} onChange={e => updateSettings({ [key]: Number(e.target.value) })} style={{ ...inputStyle, width: "auto", padding: "6px 8px", fontSize: 13 }}>
      {Array.from({ length: 24 }, (_, h) => <option key={h} value={h}>{String(h).padStart(2, "0")}:00</option>)}
    </select>
  );
  return (
    <div>
      <SectionTitle>⚙️ Ρυθμίσεις</SectionTitle>
      <div style={{ fontSize: 12, color: COLORS.sub, marginBottom: 10 }}>Οι αλλαγές αποθηκεύονται αμέσως και ισχύουν για όλη τη βάση. Κάθε ρύθμιση δείχνει στην περιγραφή της τι επηρεάζει.</div>

      <SettingsGroup title="🕒 Ωράρια & αυτοματισμοί" subtitle="Πότε τρέχει τι μέσα στη μέρα" openKey="times" cur={cur} setCur={setCur}>
        <SettingRow label="Ώρα ελέγχου κλεισίματος" hint="Πότε δημιουργούνται αυτόματα οι εργασίες «Κλείσιμο σκάφους» (μόνο σε αυτόματη λειτουργία).">{timeIn("closingCheckTime")}</SettingRow>
        <SettingRow label="Ώρα λήξης κλεισιμάτων" hint="Μετά από αυτή την ώρα, όσα κλεισίματα δεν ολοκληρώθηκαν λήγουν και ειδοποιούνται οι Base Managers.">{hourIn("closingExpireHour")}</SettingRow>
        <SettingRow label="Ώρα αναχώρησης σκάφους" hint="Μέχρι αυτή την ώρα το σκάφος που φεύγει σήμερα μετράει ακόμα ως «στη βάση» με εργασίες.">{timeIn("departureCutoffTime")}</SettingRow>
        <SettingRow label="Περιθώριο ναύλου τελευταίας στιγμής (ώρες)" hint="Αν καταχωρηθεί ναύλος αφού περάσει η ώρα αναχώρησης, δίνεται τόσος χρόνος για τις τελευταίες δουλειές.">{numIn("lateCharterGraceHours", 0, 12)}</SettingRow>
        <SettingRow label="Έναρξη ωραρίου εργασίας">{timeIn("workStart")}</SettingRow>
        <SettingRow label="Λήξη ωραρίου εργασίας" hint="Το ωράριο καθορίζει τον υπολογισμό των προθεσμιών — καμία προθεσμία δεν πέφτει εκτός αυτού.">{timeIn("workEnd")}</SettingRow>
        <div style={{ padding: "10px 0", borderTop: `1px dashed ${COLORS.line}` }}>
          <div style={{ fontSize: 13.5 }}>Μη εργάσιμες μέρες</div>
          <div style={{ fontSize: 11.5, color: COLORS.sub, marginTop: 3 }}>Δεν τρέχει κατανομή και οι προθεσμίες τις προσπερνούν.</div>
          <DayPicker value={s.nonWorkingDays} onChange={v => updateSettings({ nonWorkingDays: v })} />
        </div>
        <div style={{ padding: "10px 0", borderTop: `1px dashed ${COLORS.line}` }}>
          <div style={{ fontSize: 13.5 }}>Μέρες χωρίς νέες αυτόματες εργασίες</div>
          <div style={{ fontSize: 11.5, color: COLORS.sub, marginTop: 3 }}>Προεπιλογή Κυρ+Σαβ: το Σάββατο η προτεραιότητα είναι το κλείσιμο υπαρχουσών εργασιών, όχι νέες.</div>
          <DayPicker value={s.noAutoTaskDays} onChange={v => updateSettings({ noAutoTaskDays: v })} />
        </div>
        <SettingRow label="Ώρα εβδομαδιαίας αναφοράς">{hourIn("weeklyReportHour")}</SettingRow>
        <div style={{ padding: "10px 0", borderTop: `1px dashed ${COLORS.line}` }}>
          <div style={{ fontSize: 13.5 }}>Μέρα εβδομαδιαίας αναφοράς</div>
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginTop: 6 }}>
            {DAY_NAMES_SHORT.map((n, i) => (
      ...
      <SettingsGroup title="🤖 AI κατανομή εργασιών" subtitle="Πόσο και πώς μοιράζει το σύστημα" openKey="ai" cur={cur} setCur={setCur}>
        <SettingRow label="Μέγιστες εργασίες ανά άτομο/μέρα" hint="Ανώτατο όριο που δίνεται στο AI κατά την ημερήσια κατανομή.">{numIn("maxTasksPerEmployee", 1, 10)}</SettingRow>
        <SettingRow label="Όριο «χαμηλού φόρτου»" hint="Με τόσες ή λιγότερες ανοιχτές εργασίες, το άτομο θεωρείται διαθέσιμο για νέα αυτόματη εργασία.">{numIn("lowLoadThreshold", 0, 5)}</SettingRow>
        <SettingRow label="Εργασίες αναμονής ανά ενεργοποίηση" hint="Πόσες εργασίες «για όταν υπάρχει κενό» ενεργοποιούνται μαζί.">{numIn("maxBacklogConvert", 1, 10)}</SettingRow>
        <SettingRow label="Ιστορικό σκάφους για το AI (μέρες)" hint="Πόσο πίσω κοιτά το AI ώστε να μην ξαναστείλει κάποιον στο ίδιο σημείο.">{numIn("boatHistoryDays", 7, 90)}</SettingRow>
      </SettingsGroup>

      <SettingsGroup title="👁 Χρόνοι εμφάνισης" subtitle="Πόσο μένουν ορατά μηνύματα και πίνακες" openKey="vis" cur={cur} setCur={setCur}>
        <SettingRow label="Ορατότητα μηνυμάτων (ώρες)" hint="Πόσο μένει ορατό ένα εσωτερικό μήνυμα από τη στιγμή αποστολής.">{numIn("noteVisibleHours", 1, 72)}</SettingRow>
        <SettingRow label="Ειδοποίηση κλεισίματος έως" hint="Ως ποια ώρα το επόμενο πρωί παραμένει ορατή η ειδοποίηση για σκάφος που δεν έκλεισε.">{hourIn("closingAlertUntilHour")}</SettingRow>
        <SettingRow label="Παράθυρο «Αναχωρήσεις & Επιστροφές» (μέρες)" hint="Πόσες μέρες μπροστά δείχνει ο πίνακας στην αρχική οθόνη.">{numIn("departuresWindowDays", 1, 30)}</SettingRow>
      </SettingsGroup>

      <SettingsGroup title="🔌 Διακόπτες αυτοματισμών" subtitle="Τι τρέχει μόνο του και τι όχι" openKey="switches" cur={cur} setCur={setCur}>
        <SettingRow label="Έλεγχος κλεισίματος: αυτόματος" hint={s.closingCheckMode === "manual" ? "Χειροκίνητο: δημιουργείται μόνο με το κουμπί στην Επισκόπηση." : `Τρέχει αυτόματα μετά τις ${s.closingCheckTime}.`}>
          <Toggle on={s.closingCheckMode !== "manual"} onChange={v => updateSettings({ closingCheckMode: v ? "auto" : "manual" })} />
        </SettingRow>
        <SettingRow label="Ημερήσια κατανομή AI" hint="Η αυτόματη μοιρασιά εργασιών στο πρώτο άνοιγμα κάθε μέρας.">
          <Toggle on={s.autoDistribution !== false} onChange={v => updateSettings({ autoDistribution: v })} />
        </SettingRow>
        <SettingRow label="Αυτόματες εργασίες χαμηλού φόρτου" hint="Δημιουργία νέων εργασιών όταν η ουρά αδειάζει.">
          <Toggle on={s.autoLowLoadTasks !== false} onChange={v => updateSettings({ autoLowLoadTasks: v })} />
        </SettingRow>
        <SettingRow label="Checklist αναχώρησης" hint="Άνοιγμα λίστας ελέγχου όταν πλησιάζει αναχώρηση σκάφους.">
          <Toggle on={s.autoDepartureChecklists !== false} onChange={v => updateSettings({ autoDepartureChecklists: v })} />
        </SettingRow>
        <SettingRow label="Αυτόματη εβδομαδιαία αναφορά" hint="Αν κλείσει, η αναφορά δημιουργείται μόνο χειροκίνητα.">
      ...
      <SettingsGroup title="📷 Φωτογραφίες" subtitle="Ποιότητα & μέγεθος αποθήκευσης" openKey="photo" cur={cur} setCur={setCur}>
        <SettingRow label="Μέγιστη διάσταση (pixels)" hint="Μεγαλύτερη τιμή = καθαρότερες φωτογραφίες, αλλά πιο αργό ανέβασμα και περισσότερος χώρος.">{numIn("photoMaxDim", 640, 2560, 160)}</SettingRow>
        <SettingRow label="Ποιότητα συμπίεσης" hint="Από 0.4 (μικρό αρχείο) έως 0.95 (υψηλή ποιότητα). Προεπιλογή 0.72.">{numIn("photoQuality", 0.4, 0.95, 0.01)}</SettingRow>
      </SettingsGroup>

      <div style={{ marginTop: 14, borderTop: `1px dashed ${COLORS.line}`, paddingTop: 12 }}>
        {!confirmReset
