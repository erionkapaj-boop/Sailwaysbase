...
// adminLoginAsUser/impersonate, απλά για ΝΕΑ εγγραφή αντί για υπάρχοντα
...
// Renamed from the old loginAsTestAccount: no longer gated to rows marked
// is_test_account — see the route's own comment for what changed and why.
// `reason` is optional but always logged (even empty), so the confirm text
// in the caller is what actually enforces "say why".
export async function adminLoginAsUser(userId, reason = "") {
  const db = requireDb();
  const { data: sessionData } = await db.auth.getSession();
  ...
  // overwrites an already-stashed admin session with a stale one. The
  // target id rides along so returning can log "impersonate_end" against
  // the right account.
  sessionStorage.setItem(
    "sf_admin_return_session",
    JSON.stringify({ access_token: current.access_token, refresh_token: current.refresh_token, target_user_id: userId })
  );
...
export async function returnToAdminSession() {
  ...
  const { access_token, refresh_token, target_user_id } = JSON.parse(raw);
  // Best-effort, before the swap: this is still the admin's own token, live
  // right up until setSession() below replaces it. Never blocks the actual
  // return — a missed log line is a much smaller problem than an admin who
  // can't get back to their own console.
  if (target_user_id) {
    fetch("/api/platform/admin/impersonate/end", {
      method: "POST",
