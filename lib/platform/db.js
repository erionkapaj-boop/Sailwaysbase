...
// ---------------------------------------------------------------------------
export async function updateDeliveryAvailability(available, crewRole = null) {
  assertNotViewing();
  const db = requireDb();
  const { error } = await db.rpc("update_delivery_availability", {
...
export async function searchDeliveryCandidates(crewRole) {
  const db = requireDb();
  const { data, error } = await db.rpc("search_delivery_candidates", { p_crew_role: crewRole });
  if (error) throw error;
  return data || [];
}
