...
export async function createUserDraft({ fullName, phone, email, crewRole }) {
  ...
  } else if (crewRole === "skipper" || crewRole === "hostess" || crewRole === "cook") {
    // license_number is nullable now that phone is the identifier — the
    // platform no longer verifies licences (and hostess/cook never had one
    // to begin with).
...
// Search
// ---------------------------------------------------------------------------
export async function searchSkippers({ startDate, endDate, portId, boatTypeId, maxPrice, gender, crewRole }) {
  const db = requireDb();
