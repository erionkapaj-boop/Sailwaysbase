...
// πραγματική signInWithPassword. Ό,τι ακολουθεί (createUserDraft/complete_registration,
...
// Auth
// ---------------------------------------------------------------------------
// 'free' | 'registered' | 'previously_used' (0094) — checked before any
// identity is created, so an old number of another account never gets as far
// as an SMS code or an Auth user.
export async function phoneRegistrationStatus(phone) {
  const db = requireDb();
  const { data, error } = await db.rpc("phone_registration_status", { p_phone: normalizePhone(phone) });
  if (error) throw error;
  return data;
}
...
// After a sign-in, whether this person runs the console — an admin coming in
// through the ordinary login lands on the admin overview, not the client home.
export async function isSignedInAdmin() {
  const db = requireDb();
  const { data: auth } = await db.auth.getUser();
  if (!auth?.user) return false;
  const { data: row } = await db.from("users").select("role, is_staff_admin").eq("id", auth.user.id).maybeSingle();
  return row?.role === "admin" || Boolean(row?.is_staff_admin);
}
...
// A failure can only be recorded from here (there is no session yet). A
// success resets the counter, so it goes through clear_login_attempts, which
// only ever resets the signed-in caller's own phone (0097) — otherwise anyone
// could unlock anyone.
async function recordLoginAttempt(phone, success) {
  const db = requireDb();
  if (success) await db.rpc("clear_login_attempts");
  else await db.from("login_attempts").insert({ phone, success: false });
}
...
export async function setPin(pin) {
  ...
  if (phone) await db.rpc("clear_login_attempts");
  // Their own PIN now — any temporary one from the admin is gone (0094).
  if (auth?.user) await db.from("users").update({ pin_change_required: false }).eq("id", auth.user.id);
}
...
// convenience wrapper.
// Admin deletion (0087): any account except the main admin and yourself.
// Soft, like every deletion here — hidden and unable to sign in, nothing
// erased — so adminRestoreAccount brings it straight back. Open requests are
// cancelled with their fee refunded as credit; returns what that did.
export async function adminDeleteAccount(userId, reason) {
  const db = requireDb();
  const { data, error } = await db.rpc("admin_delete_account", { p_user_id: userId, p_reason: reason || null });
  if (error) throw error;
  return data || {};
}

export async function adminRestoreAccount(userId) {
  const db = requireDb();
  const { error } = await db.rpc("admin_restore_account", { p_user_id: userId });
  if (error) throw error;
}
...
// An already-live client asking to add a professional profile — different
// from createUserDraft above, which only ever runs once, right after OTP,
// for a brand new account. complete_registration deliberately refuses to
// change role on a live account (0082), so this is its own explicit RPC,
// reached only from an explicit "become a professional" button.
export async function becomeProfessional({ crewRole, licenseNumber, licenseType, yearsExperience, pricePerDay }) {
  assertNotViewing();
  const db = requireDb();
  const { error } = await db.rpc("become_professional", {
  ...
  if (error) throw error;
  walletChanged(); // το δώρο εγγραφής επαγγελματία
}
...
// The person moves their account to a new phone number (0094). Needs their
// current PIN; the old number stays tied to this account for good.
// Same shape as changeMyPhone: needs the PIN, goes through the server.
// An empty email removes it (and with it, email notifications and PIN reset
// by email).
export async function changeMyEmail(email, pin) {
  assertNotViewing();
  const db = requireDb();
  const { data: sessionData } = await db.auth.getSession();
  const token = sessionData?.session?.access_token;
  if (!token) throw new Error("not_authenticated");
  const res = await fetch("/api/platform/account/change-email", {
    method: "POST",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({ email: (email || "").trim(), pin }),
  });
  const body = await res.json();
  if (!res.ok) throw new Error(body.error || "change_email_failed");
  return body;
}

export async function setMyEmailNotifications(on) {
  assertNotViewing();
  const db = requireDb();
  const { data: auth } = await db.auth.getUser();
  const { error } = await db.from("users").update({ email_notifications: Boolean(on) }).eq("id", auth.user.id);
  if (error) throw error;
}

export async function changeMyPhone(newPhone, pin) {
  assertNotViewing();
  const db = requireDb();
  const { data: sessionData } = await db.auth.getSession();
  const token = sessionData?.session?.access_token;
  if (!token) throw new Error("not_authenticated");
  const res = await fetch("/api/platform/account/change-phone", {
    method: "POST",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({ newPhone: normalizePhone(newPhone), pin }),
  });
  const body = await res.json();
  if (!res.ok) throw new Error(body.error || "change_phone_failed");
  // The session's own copy of the phone is now stale — pick up the new one.
  await db.auth.refreshSession().catch(() => {});
  return body;
}

// Which self-service reset channels actually deliver right now: SMS needs a
// provider behind Supabase Auth (platform_settings.otp_enabled), email needs
// the server's email provider keys. Neither on means only the contact form.
export async function getPinResetChannels() {
  const [sms, email] = await Promise.all([
    getOtpEnabled().catch(() => false),
    fetch("/api/platform/pin-reset/request")
      .then((r) => (r.ok ? r.json() : { email: false }))
      .then((b) => Boolean(b.email))
      .catch(() => false),
  ]);
  return { sms, email };
}
...
// Κάθε κίνηση χρημάτων αλλάζει το υπόλοιπο στη βάση, αλλά η εφαρμογή το
// κρατούσε όπως φορτώθηκε στη σύνδεση — ο επαγγελματίας αποδεχόταν μια
// δουλειά, χρεωνόταν σωστά, και το Πορτοφόλι έδειχνε ακόμα το παλιό ποσό.
// Όποια συνάρτηση κινεί χρήματα το ανακοινώνει· το AuthContext ξαναδιαβάζει.
export const WALLET_EVENT = "sf-wallet-changed";
...
// Απάντηση σε πρόταση αντικατάστασης (origin='admin_replacement') — ΔΕΝ
// κλειδώνει τίποτα σε αποδοχή, μόνο δηλώνει ενδιαφέρον· διαλέγει ο πελάτης
// (clientSelectReplacementCandidate). Για κάθε άλλη πρόταση/αίτημα
// συνεχίζουν να ισχύουν claimBookingRequest/declineBookingRequest όπως πριν.
export async function respondToReplacementOffer(requestId, skipperId, accept) {
  assertNotViewing();
  const db = requireDb();
  const { error } = await db.rpc("respond_to_replacement_offer", {
    p_request_id: requestId,
    p_skipper_id: skipperId,
    p_accept: accept,
  });
...
// Ανάκληση υποψηφιότητας σε αντικατάσταση πριν αποφασίσει ο πελάτης — ο
// υποψήφιος δεν έχει χρεωθεί, άρα δεν δεσμεύεται.
export async function withdrawReplacementCandidacy(requestId, skipperId) {
  assertNotViewing();
  const db = requireDb();
  const { error } = await db.rpc("withdraw_replacement_candidacy", {
    p_request_id: requestId,
    p_skipper_id: skipperId,
  });
  if (error) throw error;
...
export async function listMyBookingRequests() {
  ...
  const { data, error } = await db
    ...
    .eq("client_id", uid)
    // Μόνο όσα έστειλε ο ίδιος: μια πρόταση αντικατάστασης του admin έχει
    // κι αυτή τον πελάτη ως client_id, αλλά δεν είναι «αίτημα που έστειλες» —
    // φαίνεται στην κράτηση που αντικαθιστά.
    .eq("origin", "client")
    // Αίτημα που δεν πληρώθηκε δεν στάλθηκε ποτέ σε κανέναν (η αποστολή
    // απέτυχε) — δεν είναι κάτι που «έστειλες».
...
export async function listRequestPings(requestId) {
  ...
  const { data, error } = await db
    ...
    .select("*, skipper_profiles(full_name, role, rating_avg, rating_count, reliability_percentage)")
...
// Το ανοιχτό αίτημα αντικατάστασης (αν υπάρχει) για μια ακυρωμένη κράτηση —
// απλό select, το RLS του booking_requests ήδη το περιορίζει σε ό,τι ανήκει
// στον καλούντα πελάτη (ή σε admin).
export async function getOpenReplacementOffer(bookingId) {
  const db = requireDb();
  const { data, error } = await db
    .from("booking_requests")
    .select("id, expires_at, client_decide_by")
    .eq("replaces_booking_id", bookingId)
...
// Οι τωρινοί υποψήφιοι για μια αντικατάσταση — ίδια ανώνυμα στοιχεία με την
// αναζήτηση (skipper_public), κανένα όνομα ή τηλέφωνο. Μόνο ο πελάτης του
// αιτήματος βλέπει κάτι (το φιλτράρει ήδη η RPC).
export async function clientListReplacementCandidates(requestId) {
  const db = requireDb();
  const { data, error } = await db.rpc("client_list_replacement_candidates", { p_request_id: requestId });
  if (error) throw error;
  return data || [];
}

// Η τελική επιλογή: κλειδώνει την ανάθεση, χρεώνει τον επιλεγμένο επαγγελματία
// ΤΩΡΑ, γράφει την κράτηση. Οριστικό — δεν αναιρείται από εδώ.
export async function clientSelectReplacementCandidate(requestId, skipperId) {
  assertNotViewing();
  const db = requireDb();
  const { data, error } = await db.rpc("client_select_replacement_candidate", {
    p_request_id: requestId,
    p_skipper_id: skipperId,
  });
  if (error) throw error;
...
export async function adminCreditWallet(userId, amount, notes) {
  ...
  if (error) throw error;
  walletChanged(); // ο admin μπορεί να φορτώνει και τον δικό του λογαριασμό
}

// Adds (positive) or takes away (negative) with a reason the user also sees.
// Never below zero: the database refuses with insufficient_wallet.
export async function adminAdjustWallet(userId, amount, reason) {
  const db = requireDb();
  const { data, error } = await db.rpc("admin_adjust_wallet", { p_user_id: userId, p_amount: amount, p_reason: reason });
  if (error) throw error;
  walletChanged();
  return data;
}
...
// Η Επισκόπηση (0088): ό,τι κινδυνεύει να χαθεί, τι έρχεται, πώς πάει η
// αγορά — σε μία κλήση.
...
// regionId: για κρατήσεις πελατών (έχουν περιοχή, όχι λιμάνι). excludeTripOf:
// μια ακυρωμένη κράτηση — όποιος έχει ήδη ακυρώσει αυτό το ταξίδι δεν
// εμφανίζεται ξανά ως υποψήφιος.
export async function adminSearchAvailability({
  role = "skipper",
  startDate,
  endDate,
  portId = null,
  ...
  const { data, error } = await db.rpc("admin_search_availability", {
    ...
    p_port_id: portId,
    p_region_id: regionId,
    p_exclude_trip_of: excludeTripOf,
  });
  if (error) throw error;
  return data || [];
}
...
// Όλες οι υποθέσεις αντικατάστασης (ακύρωση επαγγελματία → νέα ανάθεση), με
// το στάδιο κάθε μίας υπολογισμένο από τη βάση — δεν συντηρείται χειροκίνητα
// εδώ. Χωρίς όρισμα, μόνο οι ενεργές· true φέρνει και τις ολοκληρωμένες.
export async function adminReplacementCases(includeCompleted = false) {
  const db = requireDb();
  const { data, error } = await db.rpc("admin_replacement_cases", { p_include_completed: includeCompleted });
  if (error) throw error;
  return data || [];
}

// Όλη η ιστορία μίας υπόθεσης: κάθε πρόταση που στάλθηκε, ποιος απάντησε τι
// και πότε, και η τελική κράτηση αν υπάρχει — για τη σελίδα λεπτομερειών.
export async function adminReplacementCaseDetail(bookingId) {
  const db = requireDb();
  const { data, error } = await db.rpc("admin_replacement_case_detail", { p_booking_id: bookingId });
  if (error) throw error;
  return data;
}

// Οριστικό κλείσιμο υπόθεσης χωρίς αντικαταστάτη — επιστρέφει στον πελάτη
// το αρχικό τέλος (αν δεν έχει ήδη επιστραφεί) και τον ειδοποιεί.
export async function adminCloseReplacementCase(bookingId, reason) {
  assertNotViewing();
  const db = requireDb();
  const { error } = await db.rpc("admin_close_replacement_case", { p_booking_id: bookingId, p_reason: reason });
  if (error) throw error;
}
...
export async function adminListCancellationReports() {
  ...
  if (error) throw error;
  const rows = data || [];
  const people = await adminBookingPeople(db, rows.map((r) => r.bookings));
  return rows.map((r) => ({ ...r, people: people(r.bookings) }));
}

// Who is on each side of a booking, for admin lists that used to show only a
// port and dates. A second small query rather than a PostgREST embed:
// bookings.client_id points at client_profiles and names live on users
// (0081), so the embed path would be long and fragile.
async function adminBookingPeople(db, bookings) {
  const list = bookings.filter(Boolean);
  const clientIds = [...new Set(list.map((b) => b.client_id).filter(Boolean))];
  const skipperIds = [...new Set(list.map((b) => b.skipper_id).filter(Boolean))];
  const { data: sps } = skipperIds.length
    ? await db.from("skipper_profiles").select("id, user_id, full_name").in("id", skipperIds)
    : { data: [] };
  const userIds = [...new Set([...clientIds, ...(sps || []).map((sp) => sp.user_id)])];
  const { data: users } = userIds.length
    ? await db.from("users").select("id, full_name, phone_number").in("id", userIds)
    : { data: [] };
  const userById = Object.fromEntries((users || []).map((u) => [u.id, u]));
  const spById = Object.fromEntries((sps || []).map((sp) => [sp.id, sp]));
  return (b) => {
    if (!b) return { client: null, pro: null };
    const c = userById[b.client_id];
    const sp = spById[b.skipper_id];
    const pu = sp && userById[sp.user_id];
    return {
      client: c ? { id: c.id, name: c.full_name, phone: c.phone_number } : null,
      pro: sp ? { id: sp.user_id, name: sp.full_name || pu?.full_name, phone: pu?.phone_number } : null,
    };
  };
}
...
export async function adminListBookings() {
  ...
  if (error) throw error;
  const rows = data || [];
  const people = await adminBookingPeople(db, rows);
  return rows.map((b) => ({ ...b, people: people(b) }));
}
...
// A temporary PIN for a user who forgot theirs (see the route's comment).
// Returns { phone, pin } for the admin to read out to them.
export async function adminResetPin(userId) {
  const db = requireDb();
  const { data: sessionData } = await db.auth.getSession();
  const token = sessionData?.session?.access_token;
  if (!token) throw new Error("not_authenticated");
  const res = await fetch("/api/platform/admin/reset-pin", {
    method: "POST",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({ userId }),
  });
  const body = await res.json();
  if (!res.ok) throw new Error(body.error || "reset_failed");
  return body;
}
...
// The whole account dashboard for one person, in a single call: profile,
// contact, status, requests/bookings/deliveries on both sides, wallet,
// disputes, flags, login history and a unified activity timeline (0091).
// Replaces the old adminGetUserOverview, which ran up to nine separate
// client-side queries and still missed deliveries, disputes and any history
// entirely.
export async function adminAccountDetail(userId) {
  const db = requireDb();
  const { data, error } = await db.rpc("admin_account_detail", { p_user_id: userId });
  if (error) throw error;
  return data;
}

// Corrects basic details when something was typed wrong — the same fields
// self-service already lets someone change about themselves, reachable by
// an admin on their behalf. Phone number is deliberately not here: it's the
// Supabase Auth identity, not just a column — see adminEditContact.
export async function adminUpdateProfile(userId, fields) {
  ...
  const { error } = await db.rpc("admin_update_profile", {
    ...
    p_price_per_day: fields.pricePerDay ?? null,
    p_license_number: fields.licenseNumber || null,
    p_license_type: fields.licenseType || null,
    p_years_experience: fields.yearsExperience ?? null,
  });
  if (error) throw error;
}

// Phone-number correction: needs the Supabase Auth identity kept in sync
// (signInWithPin signs in with { phone, password }), which only a service-role
// route can do — see its own comment for why this can't be a plain RPC.
export async function adminEditContact(userId, phone) {
  const db = requireDb();
  const { data: sessionData } = await db.auth.getSession();
  const token = sessionData?.session?.access_token;
  if (!token) throw new Error("not_authenticated");
  const res = await fetch("/api/platform/admin/edit-contact", {
    method: "POST",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({ userId, phone }),
  });
  const body = await res.json();
  if (!res.ok) throw new Error(body.error || "edit_contact_failed");
  return body;
}

// Χειροκίνητη αφαίρεση φωτογραφίας (0092) — όχι αυτόματο moderation, μόνο η
// δυνατότητα να τη διαχειριστεί ο admin όταν τη δει προβληματική (στοιχεία
// επικοινωνίας πάνω στη φωτογραφία που παρακάμπτουν την πλατφόρμα).
export async function adminClearPhoto(userId, reason = "") {
  const db = requireDb();
  const { error } = await db.rpc("admin_clear_photo", { p_user_id: userId, p_reason: reason || null });
  if (error) throw error;
}

// Κλείνει μια σημαία (admin_flags) αφού την εξέτασε ο admin (0093) — πριν δεν
// υπήρχε κανένας τρόπος, κι έμεναν «ανοιχτές» για πάντα.
export async function adminResolveFlag(flagId) {
  const db = requireDb();
  const { error } = await db.rpc("admin_resolve_flag", { p_flag_id: flagId });
  if (error) throw error;
}

// Ουρά ελέγχου φωτογραφιών (0094): κάθε νέα/αλλαγμένη φωτογραφία φαίνεται
// αμέσως, και ο admin την εγκρίνει ή την αφαιρεί εκ των υστέρων.
export async function adminListPhotosToReview() {
  const db = requireDb();
  const { data, error } = await db.rpc("admin_list_photos_to_review");
  if (error) throw error;
  return data || [];
}

export async function adminApprovePhoto(userId) {
  const db = requireDb();
  const { error } = await db.rpc("admin_approve_photo", { p_user_id: userId });
  if (error) throw error;
}
...
// Ο πελάτης αποσύρει μια θέση που δεν την ανέλαβε κανείς ακόμα — παίρνει
// πίσω το τέλος της (0089).
export async function cancelDeliveryRoleRequest(roleRequestId) {
  assertNotViewing();
  const db = requireDb();
  const { error } = await db.rpc("cancel_delivery_role_request", { p_role_request_id: roleRequestId });
  if (error) throw error;
  walletChanged();
}

// Ακύρωση επιβεβαιωμένης μεταφοράς, από όποια πλευρά (0089). Ίδιος κανόνας
// με τις κρατήσεις πληρώματος: επιστρέφεται το τέλος της ΑΛΛΗΣ πλευράς.
export async function cancelDeliveryBooking(deliveryBookingId, reason) {
  assertNotViewing();
  const db = requireDb();
