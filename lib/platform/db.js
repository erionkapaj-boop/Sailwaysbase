...
// One round trip for the header's notification bell — open requests plus
...
export async function getSkipperNotificationCounts(skipperId) {
  const db = requireDb();
  const { data, error } = await db.rpc("skipper_notification_counts", { p_skipper_id: skipperId }).maybeSingle();
  if (error) throw error;
  return {
...
export async function adminGetUserOverview(userId, role) {
  ...
  if (role === "skipper") {
    ...
    if (pErr) throw pErr;
    if (!profile) return { role, profile: null, bookings: [], pings: [], wallet: [], blackouts: [], lookups: null };

    const [bookings, pings, wallet, blackouts, langs, boats, ports] = await Promise.all([
      ...
      db
        ...
        .order("sent_at", { ascending: false }),
      db.from("wallet_transactions").select("*").eq("user_id", userId).order("created_at", { ascending: false }).limit(50),
      db.from("skipper_availability").select("*").eq("skipper_id", profile.id).order("start_date"),
      ...
      db.from("skipper_coverage_areas").select("ports(name)").eq("skipper_id", profile.id),
    ]);
    if (bookings.error) throw bookings.error;
    ...
    return {
      ...
      ports: (ports.data || []).map((r) => r.ports?.name).filter(Boolean),
    };
  }
