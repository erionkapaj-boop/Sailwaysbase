...
export async function listLookups() {
  const db = requireDb();
  const [languages, boatTypes, ports, nationalities] = await Promise.all([
...
// Search
// ---------------------------------------------------------------------------
export async function searchSkippers({ startDate, endDate, portId, boatTypeId, maxPrice, gender, crewRole, languageId }) {
  const db = requireDb();
...
export async function listMyPings(skipperId) {
  ...
  const { data, error } = await db
    ...
    .select(
      "*, booking_requests(*, ports(name), boat_types(name), client_profiles(rating_avg,rating_count,reliability_percentage," +
        ...
        "nationalities(name),client_languages(languages(name))))"
    )
    .eq("skipper_id", skipperId)
