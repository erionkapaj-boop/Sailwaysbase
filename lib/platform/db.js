...
export async function signInWithPin(phone, pin) {
  ...
  // Deletion (0074) only marks users.status — it deliberately never touches
  // the PIN, so the old password keeps working at the Auth layer. Block it
  // here explicitly; the only way back in is re-registering with the same
  // phone, which revives the row (and the PIN they set that time).
