...
// Read-only "view as" overview for support/debugging — admin sees exactly
// what the target user would see on their own dashboard, without any
// session takeover or write access.
export async function adminGetUserOverview(userId, role) {
  ...
  if (role === "client") {
    ...
    return { role, profile: profile.data, requests: requests.data || [], bookings: bookings.data || [] };
  }

  if (role === "skipper") {
    const { data: profile, error: pErr } = await db.from("skipper_profiles").select("*").eq("user_id", userId).maybeSingle();
    if (pErr) throw pErr;
    if (!profile) return { role, profile: null, bookings: [], pings: [] };
    const [bookings, pings] = await Promise.all([
    ...
    return { role, profile, bookings: bookings.data || [], pings: pings.data || [] };
  }
