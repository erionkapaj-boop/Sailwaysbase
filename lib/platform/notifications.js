...
export function describeNotification(n) {
  ...
  switch (n.kind) {
    ...
    case "delivery_expired": {
      ...
      const route = d.origin && d.destination ? `${d.origin} → ${d.destination}` : "";
      return {
        title: `Κανείς δεν ανέλαβε τη μεταφορά${d.role ? ` (${labelForRole(d.role)})` : ""}`,
        body: [route, refund > 0 ? `επιστράφηκαν ${formatMoney(refund)}€` : ""].filter(Boolean).join(" · "),
      };
    }
    ...
    case "review_prompt":
      return {
        title: "Το ταξίδι ολοκληρώθηκε — άφησε αξιολόγηση",
        body: [d.port, dates(d)].filter(Boolean).join(" · "),
    ...
    case "profile_approved":
      return {
        ...
        body: "Εμφανίζεσαι πλέον στις αναζητήσεις — δήλωσε διαθεσιμότητα για να σε βρίσκουν.",
      };
