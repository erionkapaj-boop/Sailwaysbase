...
export function describeNotification(n) {
  ...
  switch (n.kind) {
    ...
    case "coverage_needed":
      return {
        title: "Ακύρωση: χρειάζεται αντικαταστάτης",
        body: [d.port, dates(d)].filter(Boolean).join(" · "),
        urgent: true,
      };
    case "replacement_candidate_available":
      return {
        ...
        body: [d.port, "διάλεξε από τις επιλογές σου"].filter(Boolean).join(" · "),
        urgent: true,
      };
    case "replacement_not_selected":
      return {
        title: "Δεν επιλέχθηκες για την αντικατάσταση",
        body: [d.port, "ο πελάτης διάλεξε άλλον επαγγελματία"].filter(Boolean).join(" · "),
      };
